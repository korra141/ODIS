...
class EpisodeRunner:
            ...
            if test_mode and len(self.test_returns) == self.args.test_nepisode:
                ...
                if 'offline' not in self.args.run_file:
                    if hasattr(self.mac.action_selector, "epsilon"):
                        self.logger.log_stat(f"epsilon", self.mac.action_selector.epsilon, self.t_env)
    ...
    def _log(self, returns, stats, prefix):
        ...
        log_dic[prefix + "return_mean"] = float(np.mean(returns))
        log_dic[prefix + "return_std"] = float(np.std(returns))
        returns.clear()
